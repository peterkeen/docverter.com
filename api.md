...
`input_files`
:   An array of input files. Each input file must be included in the package.
...
:   Use *FILE* as a custom template for the generated document. See [Templates](#templates) below for a description
    ...
    template appropriate for the output format will be used.
...
:   Use the specified file as a style reference in producing a docx file.
    ...
    are ignored, but its stylesheets are used in the new docx.
...
:   Specify bibliography database to be used in resolving
    ...
    bibliographies, just use an array.

`csl` *FILE*
:   Specify [CSL] style to be used in formatting citations and
    the bibliography.

`citation_abbreviations` *FILE*
...
If you use custom templates, you may need to revise them as docverter
changes.  We recommend tracking the changes in the default templates,
and modifying your custom templates accordingly. An easy way to do this
is to fork the docverter-templates repository
(<http://github.com/jgm/docverter-templates>) and merge in changes after each
docverter release.
...
In addition to raw HTML, docverter allows raw LaTeX, TeX, and ConTeXt to be
...
For output formats other than LaTeX, docverter will parse LaTeX `\newcommand` and
...
Docverter can automatically generate citations and a bibliography in a number of
...
By default, docverter will use a Chicago author-date format for citations
...
Original Â© 2006-2011 John MacFarlane (jgm at berkeley dot edu). Released under the
